...
- For more details see:
  ...
  - the [MultiIO Example](https://github.com/Cambridge-ICCS/FTorch/tree/main/examples).
...
- When creating `torch_tensor`s set the device to `torch_kCUDA` instead of `torch_kCPU`^[We can also use MPS, and are investigating AMD and XPU.]
- To target a specific device supply the `device_index` argument
- CPU-GPU cannot avoid data transfer. Use `MPI_GATHER()` to reduce
- For more details see:
  - the [online documentation](https://cambridge-iccs.github.io/FTorch/page/gpu.html)
  - the [GPU Example](https://github.com/Cambridge-ICCS/FTorch/tree/main/examples)
