...
- After building FTorch with `build_FTorch.sh`, the script also runs `pip install` to
  install the `pt2ts` command-line tool. This brings Python dependencies including
  `numpy` and `torch`.
